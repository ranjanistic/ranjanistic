    ...
    >
      <Card className="flex flex-col h-full overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 rounded-lg border border-border">
        ...
        <CardContent className="p-6 flex-grow">
          ...
          <CardDescription className="text-muted-foreground mb-4 text-sm min-h-[3em] line-clamp-2 font-serif">{project.shortDescription}</CardDescription>
          <div className="flex flex-wrap gap-2 mb-4">
            {project.technologies.map((tech) => (
              <Badge key={tech} variant="secondary" className="bg-accent/50 text-accent-foreground/80">
                {tech}
              </Badge>
            ))}
          </div>
        </CardContent>
        <CardFooter className="p-6 bg-muted/20 flex flex-col sm:flex-row justify-between items-center gap-4">
