...
export function ProjectCard({ project, animationDelay = 0 }: ProjectCardProps) {
    ...
    return (
        <div
            ref={ref}
            className={cn("h-full scroll-reveal-hidden", isVisible && "scroll-reveal-visible")}
                ...
                {project.imageUrls.map((imageUrl: string, i: number) => (
                    <CardHeader className="p-0 relative">
                        <Image
                            ...
                            data-ai-hint={project.imageHints?.[i] || ""}
                        />
                    </CardHeader>
                ))}
                <CardContent className="p-6 flex-grow">
                    <CardTitle className="text-2xl font-headline mb-2 text-foreground">{project.title}</CardTitle>
                    <CardDescription className="text-muted-foreground mb-4 text-sm min-h-[3em] line-clamp-2 font-sans">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                        {project.technologies.map((tech) => (
                            <Badge
                                key={tech}
                                variant="secondary"
                                className="bg-accent/30 text-accent-foreground/80 font-sans"
                            >
                                {tech}
                            </Badge>
                        ))}
