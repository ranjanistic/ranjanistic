...
function renderStorylineItem(item: ProjectStorylineItem, index: number) {
    switch (item.type) {
        case "heading":
            return (
                <h3 key={index} className="text-2xl md:text-3xl font-headline font-semibold mt-8 mb-4 text-foreground">
                    {item.content}
                </h3>
            );
        case "paragraph":
            return (
                <p key={index} className="text-lg text-muted-foreground mb-4 leading-relaxed font-sans">
                    {item.content}
                </p>
            );
        case "image":
        ...
        <SectionContainer animated bgColorClass="bg-section-projects" fullWidthBg>
            <div className="max-w-4xl mx-auto">
                <Button
                    variant="outline"
                    asChild
                    className="mb-8 font-sans border-primary/50 text-primary hover:bg-primary/10"
                >
                    <Link href="/projects">
                ...
                <article>
                    ...
                    <div className="prose prose-lg dark:prose-invert max-w-none prose-headings:font-headline prose-p:font-sans prose-li:font-sans prose-p:text-muted-foreground prose-headings:text-foreground">
                        <h2 className="text-3xl md:text-4xl font-headline font-semibold text-foreground mb-6 border-b border-border/50 pb-2">
                            Project Storyline
                        </h2>
