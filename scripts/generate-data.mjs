...
function safeParseJSON(jsonString, fieldName, itemName) {
    if (!jsonString || typeof jsonString !== "string") {
        console.warn(
            `Warning: Field '${fieldName}' for item '${itemName || ""}' is not a string or is empty. Returning empty array.`,
        );
        return [];
    }
    try {
...
try {
    ...
    if (workExpSheet) {
        ...
        allData.workExperienceData = workExpArray.map((item) => ({
            ...item,
            description: item.descriptionItems
                ? String(item.descriptionItems)
                      .split(";;")
                      .map((s) => s.trim())
                : [],
        }));
